...
def page_name_to_dict(wiki_db, name):
    ...
    rs = wiki_db.selectAndFetchAll('''
        select page_id, page_title name from page where page_title = %s and page_namespace = 0
...
class FeatureItemRelationManager:
    def __init__(self, lang_db, featured_page_generator, search_feature_page_from_page):
...
class _MusicGenreBuilder:
    def __init__(self, master_db, lang_db, other_lang_dbs):
        ...
        self.ipr_manager = ItemPageRelationManager(master_db, lang_db, other_lang_dbs)
        self.fir_manager = FeatureItemRelationManager(lang_db, \
    ...
    def generate_featured_pages(self):
        return self.lang_db.generate_records('an_page', cols=['page_id', 'infocontent'], cond='infotype = %s', \
    ...
    def build_feature_type_if_not_exists(self):
        rs = self.master_db.selectAndFetchAll('select * from feature_type where feature_type_id = %s', args=(self.feature_type_id,))
        if len(rs) == 0:
            self.master_db.updateQuery('insert into feature_type (feature_type_id, name) values(%s, %s)', \
                args=(self.feature_type_id, self.feature_type_name))
            self.master_db.commit()

    def _generate_source_page(self):
            
        links = []
        for record in self.lang_db.generate_records('an_page', cols=['infocontent'], cond='infotype = %s', \
                arg=(self.infotype,), dict_format=True):
            wiki_object = json.loads(record['infocontent'])
            if self.key in wiki_object:
                wiki_text = wiki_object[self.key]
                links.extend(find_links_from_wiki(wiki_text))

        links = list(set(links))
        pages = [ page_name_to_dict(self.lang_db, name) for name in links ]
        pages = [ p for p in pages if p is not None ]
        page_map = { p['page_id']: p for p in pages }
        pages = page_map.values() # unique
        pages = sorted(pages, key=lambda p:p['page_id'])
    ...
    def generate_insert_page(self):
        ...
        dest_page_iter = self.master_db.generate_records('item_page', cols=['page_id'], cond='lang=%s', \
