...
class PageBuilder:
    ...
    def _build_page(self):
        allowedInfoNames = [
            r['name']
            for r in self.db.selectAndFetchAll("select name from an_info")]

        def createPageInternal(title):
            return self.db.createPageByTitle(title, allowedInfoNames)

        source_all_page_iter = self.db.generate_records(
            'page',
            ['page_id', 'page_title', 'page_namespace', 'page_is_redirect'],
            order='page_id asc')
        source_page_iter = filter(
            lambda x:
                int(x['page_namespace']) == 0 and
                int(x['page_is_redirect']) == 0,
            source_all_page_iter)
        dest_page_iter = self.db.generate_records(
            'an_page', ['page_id'],
            order='page_id asc')
        ...
        pages = map(
            createPageInternal,
            (r['page_title'] for r in syncer.generate_for_insert()))

        for pageList in chunked(pages, 100):
            rt = self.db.multiInsert(
                'an_page',
                ['page_id', 'page_type', 'name', 'infotype', 'infocontent'],
                [[
                    p.id,
                    0,
                    p.title,
    ...
    def _build_cat(self):
        cat_iter = selectGenerator(
            self.db.openConn, 'page',
        ...
        for cat_list in chunked(cat_iter, 1000):
            self.db.multiInsert(
                'an_page',
                ['page_id', 'name', 'page_type', 'infotype', 'infocontent'],
